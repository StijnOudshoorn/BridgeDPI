...
class DataClass:
    def __init__(self, dataPath, pSeqMaxLen=1024, dSeqMaxLen=128, kmers=-1):
        ...
        pNameData, dNameData = {'train': [], 'valid': [], 'test': []}, {
            'train': [], 'valid': [], 'test': []}
        eSeqData, edgeLab = {}, {}
        ...
        for sub in ['train', 'valid', 'test']:
            ...
            id2am, id2go = [i.strip() for i in open(os.path.join(path, 'protein.vocab'), 'r').readlines()], [
                i.strip() for i in open(os.path.join(path, 'protein.goa.vocab'))]
            pnd, dnd = [], []
            for name, pSeq, gSeq in zip(tqdm(open(os.path.join(path, 'protein'), 'r').readlines()),
                                        open(os.path.join(path, 'protein.repr'), 'r').readlines(),
                                        open(os.path.join(path, 'protein.goa.repr'), 'r').readlines()):
            ...
            for name, smi in zip(tqdm(open(os.path.join(path, 'chem'), 'r').readlines()),
                                 open(os.path.join(path, 'chem.repr'), 'r').readlines()):
                ...
                if name not in self.d2id:
                    ...
                    dFeaData.append([graph_features.atom_features(a)
                    ...
                    tmp2, tmp3, tmp4 = np.ones(
                        (1,)), np.ones((1,)), np.ones((1,))
                    DataStructs.ConvertToNumpyArray(
                        AllChem.GetHashedMorganFingerprint(mol, 2, nBits=1024), tmp2)
            ...
            posEdge, negEdge = [i.strip().split(',')[1::2] for i in
                                open(os.path.join(path, 'edges.pos'), 'r').readlines()], [
                                   i.strip().split(',')[1::2] for i in
                                   open(os.path.join(path, 'edges.neg'), 'r').readlines()]
            posEdge, negEdge = [[self.p2id['p_' + p], self.d2id['d_' + d], 1] for d, p in posEdge], [
                [self.p2id['p_' + p], self.d2id['d_' + d], 0] for d, p in negEdge]
            ...
            invalidP = set([self.id2p[i] for i in np.arange(len(self.id2p))[
                (edgeLab[sub] > -1).sum(axis=1) == 0]])
            invalidD = set([self.id2d[i] for i in np.arange(len(self.id2d))[
                (edgeLab[sub] > -1).sum(axis=0) == 0]])
            pNameData[sub], dNameData[sub] = [i for i in pnd if i not in invalidP], [
                i for i in dnd if i not in invalidD]

        # dFeaData = [i.get_atom_features().tolist() for i in atomFeaturizer.featurize(dMolData, log_every_n=10000)]
        if kmers > 0:
            pSeqData_k = [[' '] * (kmers // 2) + seq + [' '] * (kmers // 2)
                          for seq in pSeqData]
            pSeqData_k = [[''.join(seq[i:i + kmers])
                           for i in range(len(seq))] for seq in pSeqData_k]

            dSeqData_k = [[' '] * (kmers // 2) + seq + [' '] * (kmers // 2)
                          for seq in dSeqData]
            dSeqData_k = [[''.join(seq[i:i + kmers])
                           for i in range(len(seq))] for seq in dSeqData_k]
        ...
        pSeqLen = []
        for pSeq, gSeq in zip(pSeqData, gSeqData):
            pSeq, gSeq = [self.am2id[am]
                          for am in pSeq], [self.go2id[go] for go in gSeq]
            pSeqLen.append(min(len(pSeq), pSeqMaxLen))
            pSeqTokenized.append(
                pSeq[:pSeqMaxLen] + [1] * max(pSeqMaxLen - len(pSeq), 0))
            gSeqTokenized.append(
                gSeq[:pSeqMaxLen] + [1] * max(pSeqMaxLen - len(gSeq), 0))
        ...
        if kmers > 0:
            ...
            for dSeq in dSeqData_k:
                ...
                dSeqTokenized_k.append(
                    dSeq[:dSeqMaxLen] + [1] * max(dSeqMaxLen - len(dSeq), 0))
        ...
        self.trainSampleNum, self.validSampleNum, self.testSampleNum = len(
            eSeqData['train']), len(eSeqData['valid']), len(eSeqData['test'])
        ...
        self.pSeqLen, self.dSeqLen = np.array(
            pSeqLen, dtype=np.int32), np.array(dSeqLen, dtype=np.int32)
        self.pSeqTokenized, self.gSeqTokenized = np.array(
            pSeqTokenized, dtype=np.int32), np.array(gSeqTokenized, dtype=np.int32)
        self.pPSSMFeat = np.array([np.vstack(
            [i, np.zeros((pSeqMaxLen - len(i), 20))]) for i in pPSSMData], dtype=np.int32)
        ...
        pContFeat = ctr.fit_transform(
            [''.join(i) for i in self.pSeqData]).toarray().astype('float32')
        k1, k2, k3 = [len(i) == 1 for i in ctr.get_feature_names()], [len(
            i) == 2 for i in ctr.get_feature_names()], [len(i) == 3 for i in ctr.get_feature_names()]
        ...
        pContFeat = (pContFeat - pContFeat.mean(axis=0)) / \
                    (pContFeat.std(axis=0) + 1e-8)
        self.pContFeat = pContFeat

        ctr = CountVectorizer(ngram_range=(1, 3), analyzer='char')
        dContFeat = ctr.fit_transform(
            [''.join(i) for i in self.dSeqData]).toarray().astype('float32')
        k1, k2, k3 = [len(i) == 1 for i in ctr.get_feature_names()], [len(
            i) == 2 for i in ctr.get_feature_names()], [len(i) == 3 for i in ctr.get_feature_names()]

        dContFeat[:, k1] = (dContFeat[:, k1] - dContFeat[:, k1].mean(
            axis=1).reshape(-1, 1)) / (dContFeat[:, k1].std(axis=1).reshape(-1, 1) + 1e-8)
        dContFeat[:, k2] = (dContFeat[:, k2] - dContFeat[:, k2].mean(
            axis=1).reshape(-1, 1)) / (dContFeat[:, k2].std(axis=1).reshape(-1, 1) + 1e-8)
        dContFeat[:, k3] = (dContFeat[:, k3] - dContFeat[:, k3].mean(
            axis=1).reshape(-1, 1)) / (dContFeat[:, k3].std(axis=1).reshape(-1, 1) + 1e-8)
        dContFeat = (dContFeat - dContFeat.mean(axis=0)) / \
                    (dContFeat.std(axis=0) + 1e-8)
        self.dContFeat = dContFeat

        self.dSeqTokenized = np.array(dSeqTokenized, dtype=np.int32)
        self.dGraphFeat = np.array(
            [i + [[0] * 75] * (dSeqMaxLen - len(i)) for i in dFeaData], dtype=np.int8)
    ...
    def vectorize(self, method="char2vec", amSize=16, goSize=16, atSize=16, window=25, sg=1, kmers=-1,
        ...
        if method == 'char2vec':
            ...
            model = Word2Vec(amDoc, min_count=0, window=window,
                             size=amSize, workers=workers, sg=sg, iter=10)
            ...
            for i in range(self.amNum):
                if self.id2am[i] in model.wv:
                    ...
                    print(
                        f'{self.id2am[i]} not in vocab, random initialize it...')
            ...
            model = Word2Vec(goDoc, min_count=0, window=window,
                             size=goSize, workers=workers, sg=sg, iter=10)
            ...
            for i in range(self.goNum):
                if self.id2go[i] in model.wv:
                    ...
                    print(
                        f'{self.id2go[i]} not in vocab, random initialize it...')
            ...
            model = Word2Vec(atDoc, min_count=0, window=window,
                             size=atSize, workers=workers, sg=sg, iter=10)
            ...
            for i in range(self.atNum):
                if self.id2at[i] in model.wv:
                    ...
                    print(
                        f"{self.id2at[i]} not in vocab, random initialize it...")
            ...
            if kmers > 0:
                ...
                for i in range(self.kamNum):
                    if self.id2kam[i] in model.wv:
                        ...
                        print(
                            f'{self.id2kam[i]} not in vocab, random initialize it...')
                        char2vec[i] = np.random.random((1, amSize))
                ...
                for i in range(self.katNum):
                    if self.id2kat[i] in model.wv:
                        ...
                        print(
                            f"{self.id2kat[i]} not in vocab, random initialize it...")
                        char2vec[i] = np.random.random((1, atSize))
    ...
    def random_batch_data_stream(self, batchSize=32, type='train', sampleType='PWRL', device=torch.device('cpu'),
                                 log=False):
        if sampleType == 'PWRL':
            ...
            while True:
                ...
                for i in range((len(nameData) + batchSize - 1) // batchSize):
                    ...
                    pTokenizedNames, dTokenizedNames = [self.p2id[i] for i in
                                                        random.sample(self.pNameData[type], batchSize)], [
                                                           self.d2id[i] for i in
                                                           random.sample(self.dNameData[type], batchSize * 20)]
                    tmp = edges[pTokenizedNames, :][:, dTokenizedNames]
                    validP, validD = ((tmp == 1).sum(axis=1) > 0) & (
                            (tmp == 0).sum(axis=1) > 0), (tmp >= 0).sum(axis=0) > 0
                    pTokenizedNames, dTokenizedNames = np.array(
                        pTokenizedNames)[validP].tolist(), np.array(dTokenizedNames)[validD].tolist()
                    ...
                    if len(pTokenizedNames) == 0 or len(dTokenizedNames) == 0:
                        ...
                        yield {
                                  ...
                                  "aminoSeq": torch.tensor(self.pSeqTokenized[pTokenizedNames], dtype=torch.long).to(
                                      device),
                                  "kaminoSeq": torch.tensor(self.pSeqTokenized_k[pTokenizedNames], dtype=torch.long).to(
                                      device),
                                  "aminoPSSM": torch.tensor(self.pPSSMFeat[pTokenizedNames], dtype=torch.float32).to(
                                      device),
                                  "aminoCtr": torch.tensor(self.pContFeat[pTokenizedNames], dtype=torch.float32).to(
                                      device),
                                  "goSeq": torch.tensor(self.gSeqTokenized[pTokenizedNames], dtype=torch.long).to(
                                      device),
                                  "pSeqLen": torch.tensor(self.pSeqLen[pTokenizedNames], dtype=torch.int32).to(device),
                                  "atomFea": torch.tensor(self.dGraphFeat[dTokenizedNames], dtype=torch.float32).to(
                                      device),
                                  "atomFin": torch.tensor(self.dFinprFeat[dTokenizedNames], dtype=torch.float32).to(
                                      device),
                                  "atomCtr": torch.tensor(self.dContFeat[dTokenizedNames], dtype=torch.float32).to(
                                      device),
                                  "atomGra": torch.tensor(self.dAdjMat[dTokenizedNames], dtype=torch.float32).to(
                                      device),
                                  "atomSeq": torch.tensor(self.dSeqTokenized[dTokenizedNames], dtype=torch.long).to(
                                      device),
                                  "katomSeq": torch.tensor(self.dSeqTokenized_k[dTokenizedNames], dtype=torch.long).to(
                                      device),
                                  "dSeqLen": torch.tensor(self.dSeqLen[dTokenizedNames], dtype=torch.int32).to(device),
                              }, torch.tensor(edges[pTokenizedNames, :][:, dTokenizedNames], dtype=torch.long).to(
                            device)
        elif sampleType == 'CEL':
            ...
            while True:
                ...
                for i in range((len(edges) + batchSize - 1) // batchSize):
                    ...
                    pTokenizedNames, dTokenizedNames = [
                                                           i[0] for i in samples], [i[1] for i in samples]
    ...
    def one_epoch_batch_data_stream(self, batchSize=32, type='valid', mode='predict', device=torch.device('cpu')):
        if mode == 'train':
            ...
            for i in range((len(nameData) + batchSize - 1) // batchSize):
                sampleNames = nameData[i * batchSize:(i + 1) * batchSize]
                pTokenizedNames, dTokenizedNames = [self.p2id[i] for i in sampleNames if i.startswith(
                    'p_')], [self.d2id[i] for i in sampleNames if i.startswith('d_')]
...
class DataClass_normal:
    def __init__(self, dataPath, pSeqMaxLen=1024, dSeqMaxLen=128, kmers=-1, validSize=0.2, sep=' '):
        ...
        with open(path, 'r') as f:
            while True:
                ...
                if drug not in self.d2id:
                    ...
                    tmp = np.ones((1,))
                    DataStructs.ConvertToNumpyArray(
                        AllChem.GetHashedMorganFingerprint(mol, 2, nBits=1024), tmp)
        ...
        # Finish
        print('Finished...')
        self.trainIdList, self.validIdList = train_test_split(
            range(len(eSeqData)), test_size=validSize)
        self.testIdList = []
        self.trainSampleNum, self.validSampleNum, self.testSampleNum = len(
            self.trainIdList), len(self.validIdList), len(self.testIdList)
        ...
        self.dSeqTokenized = np.array(dSeqTokenized, dtype=np.int32)
        self.dGraphFeat = np.array(
            [i[:dSeqMaxLen] + [[0] * 75] * (dSeqMaxLen - len(i)) for i in dFeaData], dtype=np.int8)
        self.dFinprFeat = np.array(dFinData, dtype=np.float32)
    ...
    def change_seed(self, seed, validSize=0.2):
        self.trainIdList, self.validIdList = train_test_split(
            range(len(self.eSeqData)), test_size=validSize, random_state=seed)
        self.testIdList = []
        self.trainSampleNum, self.validSampleNum, self.testSampleNum = len(
            self.trainIdList), len(self.validIdList), len(self.testIdList)
    ...
    def new_one_epoch_batch_data_stream(self, dataPath, batchSize=32, mode='predict', sep=' ',
                                        device=torch.device('cpu'), cache=None):
        # Open files and load data
        print('Loading the raw data...')
        p2id, id2p = {}, []
        d2id, id2d = {}, []
        ...
        if cache is not None and os.path.exists(cache):
            ...
            for pSeq in tqdm(pSeqData):
                ...
                pSeqTokenized.append(
                    pSeq[:self.pSeqMaxLen] + [1] * max(self.pSeqMaxLen - len(pSeq), 0))
            ...
            pSeqLen, dSeqLen = np.array(
                pSeqLen, dtype=np.int32), np.array(dSeqLen, dtype=np.int32)
            pSeqTokenized = np.array(pSeqTokenized, dtype=np.int32)

            ctr = self.pContFeatVectorizer['transformer']
            pContFeat = ctr.transform(
                [''.join(i) for i in pSeqData]).toarray().astype('float32')
            k1, k2, k3 = [len(i) == 1 for i in ctr.get_feature_names()], [len(
                i) == 2 for i in ctr.get_feature_names()], [len(i) == 3 for i in ctr.get_feature_names()]
            ...
            pContFeat[:, k3] = (pContFeat[:, k3] - pContFeat[:, k3].mean(
                axis=1).reshape(-1, 1)) / (pContFeat[:, k3].std(axis=1).reshape(-1, 1) + 1e-8)
            pContFeat = (
                                pContFeat - self.pContFeatVectorizer['mean']) / self.pContFeatVectorizer['std']

            dSeqTokenized = np.array(dSeqTokenized, dtype=np.int32)
            dGraphFeat = np.array(
                [i[:self.dSeqMaxLen] + [[0] * 75] * (self.dSeqMaxLen - len(i)) for i in dFeaData], dtype=np.int8)
    ...
    def single_data_stream(self, drug, protein, pSeqMaxLen=None, dSeqMaxLen=None, mode='predict',
