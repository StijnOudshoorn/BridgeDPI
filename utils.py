...
class DataClass:
    def __init__(self, dataPath, pSeqMaxLen=1024, dSeqMaxLen=128, kmers=-1):
        ...
        for sub in ['train', 'valid', 'test']:
            ...
            posEdge, negEdge = [i.strip().split(',')[1::2] for i in open(os.path.join(path, 'edges.pos'), 'r').readlines()], [
                i.strip().split(',')[1::2] for i in open(os.path.join(path, 'edges.neg'), 'r').readlines()]
        ...
        if sampleType == 'PWRL':
            ...
            while True:
                ...
                for i in range((len(nameData)+batchSize-1)//batchSize):
                    ...
                    pTokenizedNames, dTokenizedNames = [self.p2id[i] for i in random.sample(self.pNameData[type], batchSize)], [
                        self.d2id[i] for i in random.sample(self.dNameData[type], batchSize*20)]
